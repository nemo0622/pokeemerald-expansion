...
static const struct MenuAction sStartMenuItems[] =
{
    ...
    [MENU_ACTION_ROTOM_PHONE]     = {gText_MenuRotomPhone, {.u8_void = StartMenuPokeNavCallback}},
...
void SetDexPokemonPokenavFlags(void) // unused
{
    ...
    FlagSet(FLAG_SYS_ROTOM_PHONE_GET);
}
...
static void BuildNormalStartMenu(void)
{    
    if (FlagGet(FLAG_SYS_ROTOM_PHONE_GET) == TRUE)
        AddStartMenuAction(MENU_ACTION_ROTOM_PHONE);
...
{
    const u8 *suffix;
    const u8 UNUSED *timeofday;
    u8 UNUSED alignedSuffix[16];
    u8 UNUSED str[0x20];
    u8* ptr;
    u8 convertedHours;
...
static void ShowSaveInfoWindow(void)
{
    ...
    u32 xOffset;
    u32 yOffset;
    const u8 UNUSED *suffix;
    u8 UNUSED *alignedSuffix = gStringVar3;
