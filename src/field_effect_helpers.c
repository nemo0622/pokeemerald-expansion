...
u32 FldEff_Shadow(void)
{
    ...
    if (graphicsInfo->shadowSize == SHADOW_SIZE_S) // override small shadows to be medium
        spriteId = CreateSpriteAtEnd(gFieldEffectObjectTemplatePointers[sShadowEffectTemplateIds[SHADOW_SIZE_M]], 0, 0, 0x94 + 1); // higher = farther back; shadows should be behind object events
    else
