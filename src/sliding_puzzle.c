...
{
    const u8 color[3] = { TEXT_COLOR_TRANSPARENT, TEXT_COLOR_WHITE, TEXT_COLOR_DARK_GRAY };

    FillWindowPixelBuffer(WIN_INSTRUCTIONS_BAR, PIXEL_FILL(TEXT_COLOR_DARK_GRAY));
...
static void RotateTile(u8 rotDir)
{
    ...
    else // fuck uhhh idk. just rotate clockwise. eff it -nemo
    {
        affineAnimation = sprite->sOrientation + 8;
        sprite->sOrientation++;
        sprite->sOrientation = sprite->sOrientation % ORIENTATION_MAX;
