...
// NEMO NOTES
// Species 27258 is invalid, blocking screen from loading. WHY?
// I'm gonna assume this is something to do with the MonChoiceData.species loading wrong stuff
// Maybe go through and manually force it to always load SPECIES_ROWLET just to see?

// If that works, maybe remove MonChoiceData and just turn sStarterChoices into an array of u16's i guess
// The u16's will just be the species! Just always assume level 5 and ITEM_NONE ig lol
...
//
static const u16 sStarterChoices[9] = 
{
    [BALL_TOP_FIRST]        = SPECIES_NONE,
    [BALL_TOP_SECOND]       = SPECIES_NONE,
    [BALL_MIDDLE_FIRST]     = SPECIES_ROWLET,

    [BALL_TOP_THIRD]        = SPECIES_NONE,
    [BALL_TOP_FOURTH]       = SPECIES_NONE,
    [BALL_MIDDLE_THIRD]     = SPECIES_POPPLIO,

    [BALL_MIDDLE_SECOND]    = SPECIES_LITTEN,
    [BALL_BOTTOM_FIRST]     = SPECIES_NONE,
    [BALL_BOTTOM_SECOND]    = SPECIES_NONE,
};
...
static void CreateHandSprite()
{
    ...
    for(i=0; i<9; i++)
    {
        if(sStarterChoices[i] == SPECIES_NONE) // Choose Non Empty Slot To Start In
            continue;
    ...
    SampleUi_DrawMonIcon(sStarterChoices[sBirchCaseDataPtr->handPosition]);
...
static void CreatePokeballSprites()
{
    ...
    {
        u16 x, y;
        if(sStarterChoices[i] == SPECIES_NONE)
            continue;
...
static void BirchCase_GiveMon() // Function that calls the GiveMon function pulled from Expansion by Lunos and Ghoulslash
{
    ...
    VarSet(VAR_STARTER_MON, sStarterChoices[sBirchCaseDataPtr->handPosition]);
    gSpecialVar_Result = ScriptGiveMon(sStarterChoices[sBirchCaseDataPtr->handPosition], 5, \
                ITEM_NONE);
}
...
static void PrintTextToBottomBar(u8 textId)
{
    ...
    u16 species = sStarterChoices[sBirchCaseDataPtr->handPosition];
    ...
    if(sStarterChoices[sBirchCaseDataPtr->handPosition] == SPECIES_NONE)
    {
        PutWindowTilemap(WINDOW_BOTTOM_BAR);
        CopyWindowToVram(WINDOW_BOTTOM_BAR, 3);
        return;
...
static void Task_BirchCaseMain(u8 taskId)
{
    ...
    if(JOY_NEW(A_BUTTON))
    {
        if(sStarterChoices[sBirchCaseDataPtr->handPosition] != SPECIES_NONE) // If spot empty don't go to next control flow state
        {
            PlaySE(SE_SELECT);
