...
{
    if ((ROAMER(0)->species == SPECIES_LATIAS && GetSetPokedexFlag(SpeciesToNationalPokedexNum(SPECIES_LATIOS), FLAG_GET_SEEN))
    || (ROAMER(0)->species == SPECIES_LATIOS && GetSetPokedexFlag(SpeciesToNationalPokedexNum(SPECIES_LATIAS), FLAG_GET_SEEN))){
        SetRoamerInactive(0);
    }
    else{
        bool16 createLatios = ROAMER(0)->species == SPECIES_LATIAS;
        DeactivateAllRoamers();
        ClearRoamerLocationHistory(0);
        if(createLatios)
        {
            CreateInitialRoamerMon(0, SPECIES_LATIOS, 50);
        }
        else
        {
            CreateInitialRoamerMon(0, SPECIES_LATIOS, 50);
