...
static void ShowPostBattleHintText(void)
{
    ...
    while (!i)
    {
        ...
        case HINT_REMAINING_ITEMS:
            ...
            {
                if (events[i].graphicsId == OBJ_EVENT_GFX_GEN3_ITEM_BALL && events[i].x != SHRT_MAX && events[i].y != SHRT_MAX)
                    textIndex++;
            }
            i = 1;
            break;
...
void LoadBattlePyramidFloorObjectEventScripts(void)
{
    ...
    {
        if (events[i].graphicsId != OBJ_EVENT_GFX_GEN3_ITEM_BALL)
...
static bool8 TrySetPyramidObjectEventPositionAtCoords(u8 objType, u8 x, u8 y, u8 *floorLayoutOffsets, u8 squareId, u8 objectEventId)
{
    ...
    for (i = 0; i < mapHeader->events->objectEventCount; i++)
    {
        ...
        if (objType != OBJ_TRAINERS || mapHeader->events->objectEvents[i].graphicsId == OBJ_EVENT_GFX_GEN3_ITEM_BALL)
        {
            if (objType != OBJ_ITEMS || mapHeader->events->objectEvents[i].graphicsId != OBJ_EVENT_GFX_GEN3_ITEM_BALL)
                continue;
        }
        ...
        if (j == objectEventId)
        {
            ...
            if (floorEvents[objectEventId].graphicsId != OBJ_EVENT_GFX_GEN3_ITEM_BALL)
            {
