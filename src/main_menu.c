...
static u32 InitMainMenu(bool8 returningFromOptionsMenu)
{
    ...
    ResetPaletteFade();
    // LoadPalette(sMainMenuBgPal, BG_PLTT_ID(0), PLTT_SIZE_4BPP); // Commented out to hide blue background from loading
...
static void Task_DisplayMainMenu(u8 taskId)
{
    ...
    // Loads Archie/Mudskip main menu in ui_main_meni.c
    // Commented out cause I wanna do my own stuff I think
...
static void Task_NewGameBirchSpeechSub_InitPokeBall(u8 taskId)
{
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, SPECIES_MANTINE);
...
{
    return CreateMonPicSprite_Affine(SPECIES_MANTINE, FALSE, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
