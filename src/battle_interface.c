...
static u8 *AddTextPrinterAndCreateWindowOnHealthboxWithFont(const u8 *str, u32 x, u32 y, u32 bgColor, u32 *windowId, u32 fontId, bool32 isHP)
{
    ...
    FillWindowPixelBuffer(winId, PIXEL_FILL(6));
...
{
    return AddTextPrinterAndCreateWindowOnHealthboxWithFont(str, x, y, 6, windowId, FONT_SMALL, isHP);
}
...
void ArrowsChangeColorLastBallCycle(bool32 showArrows)
{
    ...
    if (gBattleStruct->ballSpriteIds[1] == MAX_SPRITES)
        return;
    paletteNum *= 16;
    pltArrow = (struct PlttData *)&gPlttBufferFaded[paletteNum + 4];  // Arrow color is in idx 4Add commentMore actions
