...
static void DisplayPartyPokemonBarDetail2(u8 windowId, const u8 *str, u8 color, const u8 *align)
{
    ...
    AddTextPrinterParameterized3(windowId, GetFontIdToFit(str, FONT_SMALL, 0, 50), align[0] + 4, align[1], sFontColorTable[color], 0, str);
}
...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    if(sPartyMenuInternal->numActions < 4)
    {
        ...
        if (CanLearnTeachableMove(GetMonData(&mons[slotId], MON_DATA_SPECIES), MOVE_SURF) && FlagGet(FLAG_BADGE04_GET) && sPartyMenuInternal->numActions < 4 && !MonKnowsMove(&mons[slotId], MOVE_SURF) && CheckBagHasItem(ITEM_HM03, 1))
        {
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 4 + MENU_FIELD_MOVES);
        }
        if (CanLearnTeachableMove(GetMonData(&mons[slotId], MON_DATA_SPECIES), MOVE_FLY) && FlagGet(FLAG_BADGE07_GET) && sPartyMenuInternal->numActions < 4 && !MonKnowsMove(&mons[slotId], MOVE_FLY) && CheckBagHasItem(ITEM_HM02, 1))
        {
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 5 + MENU_FIELD_MOVES);
        }
        if (CanLearnTeachableMove(GetMonData(&mons[slotId], MON_DATA_SPECIES), MOVE_DIVE) && FlagGet(FLAG_BADGE05_GET) && sPartyMenuInternal->numActions < 4 && !MonKnowsMove(&mons[slotId], MOVE_DIVE) && CheckBagHasItem(ITEM_HM08, 1))
        {
        ...
        if (CanLearnTeachableMove(GetMonData(&mons[slotId], MON_DATA_SPECIES), MOVE_FLASH) && FlagGet(FLAG_BADGE03_GET) && sPartyMenuInternal->numActions < 4 && !MonKnowsMove(&mons[slotId], MOVE_FLASH) && CheckBagHasItem(ITEM_HM05, 1))
        {
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 1 + MENU_FIELD_MOVES);
        }
        if (CanLearnTeachableMove(GetMonData(&mons[slotId], MON_DATA_SPECIES), MOVE_ROCK_SMASH) && FlagGet(FLAG_BADGE02_GET) && sPartyMenuInternal->numActions < 4 && !MonKnowsMove(&mons[slotId], MOVE_ROCK_SMASH) && CheckBagHasItem(ITEM_HM06, 1))
        {
            AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 2 + MENU_FIELD_MOVES);
        }
        if (CanLearnTeachableMove(GetMonData(&mons[slotId], MON_DATA_SPECIES), MOVE_STRENGTH) && FlagGet(FLAG_BADGE06_GET) && sPartyMenuInternal->numActions < 4 && !MonKnowsMove(&mons[slotId], MOVE_STRENGTH) && CheckBagHasItem(ITEM_HM04, 1))
        {
    ...
    if (!InBattlePike())
    {
        ...
        // view Pokédex from party menu - removed cause bugs for now
