...
static void SetPartyMonFieldSelectionActions(struct Pokemon *mons, u8 slotId)
{
    ...
    if (sPartyMenuInternal->numActions < 5 && CanLearnTeachableMove(GetMonData(&mons[slotId], MON_DATA_SPECIES, 0), MOVE_FLY) && CheckBagHasItem(ITEM_HM_FLY, 1)) 
    {
        AppendToList(sPartyMenuInternal->actions, &sPartyMenuInternal->numActions, 5 + MENU_FIELD_MOVES);
    }
    // If Mon can learn HM05 and action list consists of < 4 moves, add FLASH to action list
    else if (sPartyMenuInternal->numActions < 5 && CanLearnTeachableMove(GetMonData(&mons[slotId], MON_DATA_SPECIES, 0), MOVE_FLASH) && CheckBagHasItem(ITEM_HM_FLASH, 1)) 
