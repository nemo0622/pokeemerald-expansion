...
static void RotomPhone_RotomRealityMenu_PrintTips(void);
...
static bool32 RotomPhone_StartMenu_UnlockedFunc_False(void);
...
enum RotomPhone_Overworld_FaceIconPaletteIndex
{
    ...
    PAL_ICON_ORANGE,
    PAL_ICON_BROWN,
    PAL_ICON_MONOCHROME,    // Gray
    PAL_ICON_WHITE,
    PAL_ROTOM_OUTLINE,      // Black
    PAL_ROTOM_EYE_WHITE,    // White
    PAL_ROTOM_EYE_TOP,      // This is actually red now
    PAL_ROTOM_EYE_BOTTOM,   // Blue of eyes
    PAL_ROTOM_ARC,          // Blue of Rotom's screen & projection
};
...
#define OW_FLIP_PHONE_TEXT_BG_COLOUR       1
#define OW_FLIP_PHONE_TEXT_FG_COLOUR       0
#define OW_FLIP_PHONE_TEXT_SHADOW_COLOUR   10
#define OW_ROTOM_PHONE_TEXT_BG_COLOUR      1
#define OW_ROTOM_PHONE_TEXT_FG_COLOUR      14
...
static const struct RotomPhone_MenuOptions sRotomPhoneOptions[RP_MENU_COUNT] =
{
    ...
    [RP_MENU_SAVE] =
    {
        ...
        .rotomSpeech = COMPOUND_STRING("to save your progress?"),
    ...
    [RP_MENU_DAYCARE] =
    {
        ...
        .unlockedFunc = RotomPhone_StartMenu_UnlockedFunc_False, // permanently locked
...
static void Task_RotomPhone_OverworldMenu_HandleMainInput(u8 taskId)
{
    ...
    if (JOY_NEW(A_BUTTON))
    {
        if (sRotomPhone_StartMenu->menuOverworldLoading == FALSE)
        {
            ...
            else if (menuSelectedOverworld != RP_MENU_SAVE && menuSelectedOverworld != RP_MENU_FLAG)
...
static void RotomPhone_RotomRealityMenu_SetupCB(void)
{
    ...
    case 7:
        ...
        RotomPhone_RotomRealityMenu_PrintTips();
...
static void RotomPhone_RotomRealityMenu_InitWindows(void)
{
    ...
    FillWindowPixelBuffer(RP_RR_WIN_TIP_NUM, PIXEL_FILL(TEXT_COLOR_TRANSPARENT));
    PutWindowTilemap(RP_RR_WIN_TIP_NUM);
    CopyWindowToVram(RP_RR_WIN_TIP_NUM, COPYWIN_FULL);
    FillWindowPixelBuffer(RP_RR_WIN_TIP_TEXT, PIXEL_FILL(TEXT_COLOR_TRANSPARENT));
    PutWindowTilemap(RP_RR_WIN_TIP_TEXT);
    CopyWindowToVram(RP_RR_WIN_TIP_TEXT, COPYWIN_FULL);
}
...
static void RotomPhone_StartMenu_LoadRotomFaceSpritesheet(void)
{
    ...
    if (GetSpriteTileStartByTag(rotomFace.tag) == TAG_NONE)
    {
        ...
        LoadCompressedSpriteSheet(&rotomFace);
        sRotomPhone_StartMenu->rotomFaceLastLoaded = RP_FACE_SHOCKED;
    }
        ...
        if (sRotomPhone_StartMenu->rotomFaceLastLoaded >= RP_FACE_SHOCKED_WITH)
        {
            do {
                rotomFace = Random() % randMax;
                    ...
                    rotomFace == gSprites[sRotomPhone_StartMenu->menuRotomFaceSpriteId].animNum ||
                    (rotomFace != RP_FACE_HAPPY_WITH && rotomFace != RP_FACE_SHOCKED_WITH)
                );
        }
...
static bool32 RotomPhone_StartMenu_UnlockedFunc_Shortcut(void)
{
    if (FlagGet(FLAG_SYS_POKEDEX_GET) && (!RotomPhone_StartMenu_IsRotomReality()) && !GetSafariZoneFlag())
        return TRUE;
    else
        return FALSE;
}
...
static void RotomPhone_StartMenu_SelectedFunc_Pokedex(void)
{
    RotomPhone_StartMenu_DoCleanUpAndChangeCallback(Nemo_ShowPokedexFromMenu);
}
...
#define TIP_COUNT           50
static const u8 *const sTipArray[TIP_COUNT] =
{
    // Tip "basics" - all showing up more often on Melemele Island!
    COMPOUND_STRING("If you get lost, check the\nQuest Menu for guidance!"),
    COMPOUND_STRING("If you get lost, check the\nQuest Menu for guidance!"),
    COMPOUND_STRING("Use your favorite Pokémon, not just\nthe strongest ones!"),
    COMPOUND_STRING("More areas unlock as you complete\nTrials for your Island Challenge!"),
    COMPOUND_STRING("Samson Oak studies super-strong variants\nof Pokémon called Totem Pokémon!"),
    COMPOUND_STRING("Talk to Samson Oak's Oranguru to change\ncertain game settings!"),
    COMPOUND_STRING("Make sure to talk to everyone in Alola!\nSome might even have gifts for you!"),
    COMPOUND_STRING("Large signposts have extra information about\nthe surrounding environment!"),
    COMPOUND_STRING("Lots of people walk with their Pokémon\noutside of Poké balls in Alola!"),
    COMPOUND_STRING("Some Pokémon are visible in the tall grass.\nAlways be on the lookout for rare species!"),
    // More advanced / fun tips below
    COMPOUND_STRING("The four islands of Alola each have a\nGuardian Deity, like Tapu Koko on Melemele!"),
    COMPOUND_STRING("You shouldn't disturb sleeping Seel in the\nAlola Region - they're a protected species!"),
    COMPOUND_STRING("Pokémon need to hold Z-Crystals to use\nZ-Moves or access special Z-Forms."),
    COMPOUND_STRING("The Aether Foundation was founded by\nLusamine from the Kanto Region."),
    COMPOUND_STRING("Some people in Alola will ask you for favors.\nThey'll often reward you for helping!"),
    COMPOUND_STRING("Mass Pokémon migrations have changed the\nmakeup of the Alola Region's ecosystems."),
    COMPOUND_STRING("Try Fishing for Pokémon where you can!\nSometimes, you'll even pull up treasure."),
    COMPOUND_STRING("A talented artist named Zezitra made\nthe Alola Region Map art!"),
    COMPOUND_STRING("A talented artist named Zaebucca painted\nlots of Alola Region artwork!"),
    COMPOUND_STRING("A talented artist named SageDeoxys made\nart of many Alolan Pokémon!"),
    COMPOUND_STRING("You can change your adventure's difficulty\nwith Samson Oak's Oranguru in your room."),
    COMPOUND_STRING("Samson Oak, your grandpa, runs a Pokémon\nSanctuary service in his garden!"),
    COMPOUND_STRING("Golden stickers, called Totem Stickers, can\nbe exchanged for powerful Pokémon."),
    COMPOUND_STRING("There are rumors of tiny Zygarde Cells\nscattered around Alola. Try finding them!"),
    COMPOUND_STRING("Corsola comes in a wide variety of colors.\nTry catching them all!"),
    COMPOUND_STRING("Café's are located in every Pokémon Center.\nThey sell a lot of helpful drinks!"),
    COMPOUND_STRING("Interact with Game Tables in most cities\nto play the Pokémon TCG Minigame!"),
    COMPOUND_STRING("Many Pokémon ridden with the Poké Ride\nsystem have special abilities, like Surfing!"),
    COMPOUND_STRING("The Pokédex can show you all the moves\nPokémon learn by training!"),
    COMPOUND_STRING("The Pokédex can tell you where Pokémon\nmay be found!"),
    COMPOUND_STRING("The Pokédex can tell you the base stats\nof Pokémon and show their forms."),
    COMPOUND_STRING("Press A on the Region Map to view more\ninformation about a location!"),
    COMPOUND_STRING("Doduo and Dodrio evolved in isolation in\nAlola. They're found nowhere else!"),
    COMPOUND_STRING("There is so much to do besides battling!\nLots of Minigames are found around Alola!"),
    COMPOUND_STRING("There are rumors about Ultra Wormholes\nproducing strange Pokémon..."),
    COMPOUND_STRING("Some Pokémon have special forms that can\nonly be obtained from completing Quests."),
    COMPOUND_STRING("Use the DexNav to view what Pokémon\nare in your area. You can even target them!"),
    COMPOUND_STRING("Some Pokémon are only found at certain\ntimes. Always look for new species!"),
    COMPOUND_STRING("Trial Captains share a special bond with\ntheir Totem Pokémon. They're super strong!"),
    COMPOUND_STRING("Raichu's special Alolan Form is said to\nhave inspired the first surfers!"),
    COMPOUND_STRING("Murkrow from Alola are among the smartest\nPokémon in the world. Some can use tools!"),
    COMPOUND_STRING("Seeing Ribombee in the wild is a sign\nof good weather to come!"),
    COMPOUND_STRING("Using the Poké Ride system can help\nyou get around the islands faster!"),
    COMPOUND_STRING("Rowlet are known to tuck themselves\nin their Trainer's open backpacks."),
    COMPOUND_STRING("Litten are known to burn their old\nfur off to grow it all new."),
    COMPOUND_STRING("Popplio are known to use water bubbles\nto play with other Pokémon."),
    COMPOUND_STRING("Prof. Kukui caught me as a wild Rotom in the\nSinnoh Region. The weather's better here!"),
    COMPOUND_STRING("Some Trainers give themselves special rules\nto challenge themselves. Sounds hard..."),
    COMPOUND_STRING("Different forms of Oricorio can be found\non each island. They're all so great!"),
};
...
static void RotomPhone_RotomRealityMenu_PrintTips(void)
{
    ...
    // Print "Rotom Tips" text
    StringCopy(gStringVar2, COMPOUND_STRING("Rotom Tips"));
    FillWindowPixelBuffer(RP_RR_WIN_TIP_NUM, PIXEL_FILL(TEXT_COLOR_TRANSPARENT));
    AddTextPrinterParameterized4(RP_RR_WIN_TIP_NUM, FONT_SHORT_NARROW, GetStringCenterAlignXOffset(FONT_SHORT_NARROW, gStringVar2, sRotomPhone_RotomRealityMenuWindowTemplates[RP_RR_WIN_TIP_NUM].width * 8) , 2, 0, 0, color, TEXT_SKIP_DRAW, gStringVar2);
    CopyWindowToVram(RP_RR_WIN_TIP_NUM, COPYWIN_GFX);

    // Pick a random Tip from below:
    u16 randomTip;
    if(FlagGet(FLAG_GOT_FIGHTING_Z_CRYSTAL))
        randomTip = Random() % TIP_COUNT;
    else
        randomTip = Random() % 10; // If still early-game, limit tips to basics to get them in people's heads
    StringCopy(gStringVar3, sTipArray[randomTip]);

    FillWindowPixelBuffer(RP_RR_WIN_TIP_TEXT, PIXEL_FILL(TEXT_COLOR_TRANSPARENT));

    AddTextPrinterParameterized4(RP_RR_WIN_TIP_TEXT, FONT_SHORT_NARROW, GetStringCenterAlignXOffset(FONT_SHORT_NARROW, gStringVar3, sRotomPhone_RotomRealityMenuWindowTemplates[RP_RR_WIN_TIP_TEXT].width * 8), 0, 0, 0, color, TEXT_SKIP_DRAW, gStringVar3);

    CopyWindowToVram(RP_RR_WIN_TIP_TEXT, COPYWIN_GFX);
