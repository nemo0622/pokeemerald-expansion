...
const struct TrainerClass gTrainerClasses[TRAINER_CLASS_COUNT] =
{
    ...
    TRAINER_CLASS(GUITARIST, "GUITARIST", 8, ITEM_QUICK_BALL),
    ...
    TRAINER_CLASS(MAGMA_ADMIN, "MAGMA ADMIN", 10, ITEM_PREMIER_BALL),
    TRAINER_CLASS(RIVAL, "{PKMN} TRAINER", 15, ITEM_FRIEND_BALL),
    TRAINER_CLASS(BUG_CATCHER, "BUG CATCHER", 4, ITEM_NEST_BALL),
    TRAINER_CLASS(PKMN_RANGER, "{PKMN} RANGER", 12, ITEM_GREAT_BALL),
...
static void ReturnFromBattleToOverworld(void)
{
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_ROAMER)
    {
        ...
        if ((gBattleOutcome == B_OUTCOME_WON) || gBattleOutcome == B_OUTCOME_CAUGHT) // Bug: When Roar is used by roamer, gBattleOutcome is B_OUTCOME_PLAYER_TELEPORTED (5).
            ...
            NextRoamer();
            // SetRoamerInactive(gEncounteredRoamerIndex); // NOTE: added by 1.9.0
    }
