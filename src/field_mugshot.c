...
#define MUGSHOT_X 176 + 32
#define MUGSHOT_Y 49  + 32
...
void CreatePlayerFieldMugshot(struct ScriptContext *ctx)
{
    u16 id = 1;
    u16 emote = VarGet(ScriptReadHalfword(ctx));
...
void _CreateFieldMugshot(u32 id, u32 emote)
{
    ...
    sheet.data = (sFieldMugshots[id][emote].gfx != NULL ? sFieldMugshots[id][emote].gfx : sFieldMugshotGfx_Elio_Normal);
    pal.data = (sFieldMugshots[id][emote].pal != NULL ? sFieldMugshots[id][emote].pal : sFieldMugshotPal_Elio_Normal);
