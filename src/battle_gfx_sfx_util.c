...
void BattleLoadMonSpriteGfx(struct Pokemon *mon, u32 battler)
{
    ...
    UniquePalette(paletteOffset, species, monsPersonality, isShiny);
    CpuCopy32(gPlttBufferFaded + paletteOffset, gPlttBufferUnfaded + paletteOffset, 32);
    UniquePalette(0x80 + battler * 16, species, monsPersonality, isShiny);
...
{
    if (B_ENEMY_MON_SHADOW_STYLE >= GEN_4 && !FlagGet(FLAG_HIDE_BATTLE_SHADOWS))
    {
...
void SpriteCB_EnemyShadow(struct Sprite *shadowSprite)
{
    ...
    s8 xOffset = 0, UNUSED yOffset = 0, size = 0;
    ...
    else if (transformSpecies != SPECIES_NONE)
    {
        ...
        invisible = (B_ENEMY_MON_SHADOW_STYLE >= GEN_4 && !FlagGet(FLAG_HIDE_BATTLE_SHADOWS))
                  ? gSpeciesInfo[transformSpecies].suppressEnemyShadow
                  : gSpeciesInfo[transformSpecies].enemyMonElevation == 0;
    }
    else if (B_ENEMY_MON_SHADOW_STYLE >= GEN_4 && !FlagGet(FLAG_HIDE_BATTLE_SHADOWS))
    {
        u16 species = SanitizeSpeciesId(gBattleMons[battler].species);
