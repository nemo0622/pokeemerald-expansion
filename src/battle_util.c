...
void HandleAction_UseMove(void)
{
    ...
    else if ((GetBattlerSide(gBattlerAttacker) == GetBattlerSide(gBattlerTarget))
            ...
            case ABILITY_SCAVENGER:
                 ...
                 && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK))
                {
                    ...
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
                    gBattleMoveDamage *= -1;
                    BattleScriptPushCursorAndCallback(BattleScript_ScavengerActivates);
                    effect++;
                }
                break;
    ...
    case ABILITYEFFECT_WOULD_ABSORB:
        if (move != MOVE_NONE)
        {
            ...
            case ABILITY_EARTH_EATER:
                ...
                break;
            case ABILITY_ATMOSPHERIC:
                if(moveType == TYPE_WATER || moveType == TYPE_FLYING)
                    effect = 1;
                break;
            }
...
static inline u32 CalcAttackStat(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 isCrit, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk)
{
    ...
    case ABILITY_BLAZE:
        if (moveType == TYPE_FIRE && gBattleMons[battlerAtk].hp <= (gBattleMons[battlerAtk].maxHP / 3))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    case ABILITY_CELESTIAL_BOOST:
        if (moveType == TYPE_FIRE || moveType == TYPE_GHOST)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        break;
    ...
    case HOLD_EFFECT_THICK_CLUB:
        if ((atkBaseSpeciesId == SPECIES_CUBONE || atkBaseSpeciesId == SPECIES_MAROWAK || atkBaseSpeciesId == SPECIES_MAROWAK_ALOLAN || atkBaseSpeciesId == SPECIES_VULLABY || atkBaseSpeciesId == SPECIES_MANDIBUZZ) && IS_MOVE_PHYSICAL(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(2.0));
        break;
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
    ...
    else if (defType == TYPE_STEEL && abilityAtk == ABILITY_CORROSION
        && mod == UQ_4_12(0.0))
    {
        mod = UQ_4_12(2.0);
        if (recordAbilities)
    ...
    if (moveType != TYPE_MYSTERY)
    {
        ...
        if ((modifier <= UQ_4_12(1.0)  &&  abilityDef == ABILITY_WONDER_GUARD)
         ...
         || (moveType == TYPE_WATER    && (abilityDef == ABILITY_WATER_ABSORB
                                       ...
                                       || abilityDef == ABILITY_ATMOSPHERIC
