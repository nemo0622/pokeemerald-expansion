...
static void CreateMinigameSprites(u8 taskId)
{
    ...
    else if (GetMonAbility(&gPlayerParty[0]) == ABILITY_ILLUMINATE) // if Pokémon has Illuminate, boost treasure odds!
    {
        if((Random() % 100) < 60) // 60% chance for Treasure to spawn when Pokémon has Illuminate!
            CreateTreasureSprite(taskId);
    }
...
static void SetFishingTreasureItem(u8 rod)
{
    ...
    item = random + offset;
    if(((Random() % 100) + 1) <= 15) // roughly 15% chance at all times to just pull a Wishing Star
    {
        gSpecialVar_ItemId = ITEM_WISHING_STAR;
    }
    else if(!FlagGet(FLAG_GOT_SCALD_TM_FISHING_REWARD) && (rod == GOOD_ROD || rod == SUPER_ROD))
    {
        if(((Random() % 100) + 1) <= 10) // 10% chance to get scald
        {
            FlagSet(FLAG_GOT_SCALD_TM_FISHING_REWARD);
            gSpecialVar_ItemId = ITEM_TM56;
        }
        else
...
static void SpriteCB_FishingBar(struct Sprite *sprite)
{
    ...
    if (sprite->sBarPosition == 0 && sprite->sBarDirection == FISH_DIR_LEFT && sprite->sBarSpeed > 0)
    {
        ...
        if (GetMonAbility(&gPlayerParty[0]) == ABILITY_ROCK_HEAD || GetMonAbility(&gPlayerParty[0]) == ABILITY_SOLID_ROCK || GetMonAbility(&gPlayerParty[0]) == ABILITY_STURDY)
            sprite->sBarSpeed /= (FISHING_BAR_BOUNCINESS + 0.7);
        else
