...
void CreateBoxMon(struct BoxPokemon *boxMon, u16 species, u8 level, u8 fixedIV, u8 hasFixedPersonality, u32 fixedPersonality, u8 otIdType, u32 fixedOtId)
{
    ...
    else // Player is the OT
    {
        ...
        else
        {
            ...
            if (CheckBagHasItem(ITEM_SHINY_CHARM, 2)) // if a second shiny charm, add more rerolls
                totalRerolls += I_SHINY_CHARM_ADDITIONAL_ROLLS;
    ...
    for (i = 0; i < ITEM_EFFECT_ARG_START; i++)
    {
        ...
        case 3:
            ...
            if ((itemEffect[i] & ITEM3_LEVEL_UP)
             ...
             ((FlagGet(FLAG_LEVEL_CAPS_ENABLED) && GetMonData(mon, MON_DATA_LEVEL) < GetCurrentLevelCap()) || (!FlagGet(FLAG_LEVEL_CAPS_ENABLED))))
            {
...
{
    if ((gHitMarker & HITMARKER_NO_ANIMATIONS && !(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK))) || (gBattleTypeFlags & BATTLE_TYPE_DOUBLE && !(gBattleTypeFlags & (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK))))
