...
static void DexNavGenerateMoveset(u16 species, u8 searchLevel, u8 encounterLevel, u16* moveDst)
{
    ...
    if (genMove)
    {
        u8 numEggMoves = GetEggMovesSpecies(species, eggMoveBuffer);
        if (numEggMoves != 0)
...
{
    if (species == SPECIES_NONE || species > NUM_SPECIES || MapHasNoEncounterData())
...
static void DrawSpeciesIcons(void)
{
    ...
    for (i = 0; i < HIDDEN_WILD_COUNT; i++)
    {
        ...
        if (FlagGet(FLAG_SYS_DETECTOR_MODE))
            TryDrawIconInSlot(species, x, y);
       else if (species == SPECIES_NONE || species > NUM_SPECIES || MapHasNoEncounterData())
